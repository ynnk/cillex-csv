...
define([
  // These are path alias that we configured in our main.js
	...
	'numeric',
    'pdgconst'
], function($, _, Backbone, Mousetrap, Semantic, Cello, Gviz, Materials, moment, Polyfill, Numeric, Const){
...
var THEMES = {
        ...
        camouflage: {  top  : "rgb(93, 112, 92)",
                top_menu:  "#FFF",
                
                right : "rgb(93, 112, 92)",
                viz  : "rgb(122, 143, 121)",
                controls : "rgb(93, 112, 92)",
                top_inverted: true,
                right_inverted: true
            },
    }
...
function setTheme(name){
    ...
    $("#padagraph-top .menu").removeClass("inverted")
    $("#padagraph-top .menu .input").removeClass("inverted")
    $("#padagraph-right .fitted.pointing.menu").removeClass("inverted")

    var theme = THEMES[name];
    $("#padagraph-top").css('background', theme.top)
    $("#padagraph-top .menu .menu").css('background', theme.top_menu)
    $("#padagraph-right").css('background', theme.right);
    $("#padagraph-viz").css('background', theme.viz);
    $("padagraph-gviz")[0].gviz.set_clear_color(theme.viz);
    $("padagraph-controls .menu").css( "background", theme.controls );

    if (theme.top_inverted){
        $("#padagraph-top .menu").addClass("inverted")
        $("#padagraph-top .menu .input").addClass("inverted")
        $("#padagraph-top .menu .menu").addClass("inverted")
    }
    if (theme.right_inverted){
        $("#padagraph-right .fitted.pointing.menu").addClass("inverted")
    }
    _window_resized();
}
...
function setFullscreen(fullscreen){
    ...
    if (fullscreen) {
        $('#padagraph-left').removeClass("twelve wide column").addClass('sixteen wide column');
        $('#padagraph-right').hide()
    }
    else {
        $('#padagraph-left').removeClass("sixteen wide column").addClass('twelve wide column');
        $('#padagraph-right').show()
    }
...
function initUI(){
    ...
    // panel tabs
    $('#padagraph-right .item').tab();

    // cubes dropdown
    //-
        //$('#padagraph-top .ui.dropdown')
          //.dropdown({
...
var _window_resized = function(){
    ...
    $("#padagraph-viz").height(size -52);
    $("#padagraph-right").height(size- 6);

    $('padagraph-gviz')[0].gviz.resize_rendering();
}
